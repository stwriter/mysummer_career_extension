<template>
  <div v-if="show" class="deposit-modal-overlay" @click.self="close">
    <div class="deposit-modal">
      ...
      <form @submit.prevent="handleSubmit" class="form">
        ...
        <button
          type="submit"
          class="submit-btn"
          :disabled="!amount || amount <= 0 || amount > walletBalance || processing"
        >
          {{ processing ? 'Depositing...' : `Deposit $${formatCurrency(amount || 0)}` }}
        </button>
      </form>
    </div>
  </div>
...
const handleSubmit = async () => {
  ...
  try {
    const success = await lua.career_modules_bank.deposit(
      props.accountId,
